Start-Transcript -Path "C:\Windows\LTSvc\Scripts\WindowsServerDiscovery_Transcript.rtf" -append -NoClobber
...
$htmlfile = "C:\Windows\LTSvc\Scripts\WindowsServerDiscovery_$machine_name.html"
...
function OperatingSystemSection {
    ...
    "
    $section  | Out-File -FilePath $htmlfile -append
...
function ComputerInfo {
    ...
    $bios_output | Out-File -FilePath $htmlfile -append
}
...
Function ShortFileConfig {
    if ((Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\File System').NtfsDisable8dot3NameCreation  -eq 1){
    $section = "
        <tr>
...
<head>
    ...
    <title>Server Discovery report for [$machine_name]</title>
</head>
<body>
<h2 align=center>Server Discovery report for [$machine_name]</h2>
<table align=center border=1 width=80%>
<tr>
    <td><b><center>Discovery Task</center></b></td>
    <td><b><center>Result (Green=GOOD, Red=BAD)</center></b></td>
    <td><b><center>Notes/Fix</center></b></td>
"
...
AppCheck -AppName 'Putty' -TargetVersion "0.65"
